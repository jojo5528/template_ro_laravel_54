...
{
    public function get_emblem(Request $request, Guild $guild)
    {
        if(!empty($guild->emblem_data)){
            $data = $this->imgFromBLOB($guild->emblem_data);
            $img = imagepng($data);
        }else{
            $img = file_get_contents('https://place-hold.it/24x24?text=?');
        }
        return response($img)->header('Content-Type', 'image/png');
    }

    protected function imgFromBLOB($blob)
    {
        $im  = @gzuncompress(pack('H*', $blob));
